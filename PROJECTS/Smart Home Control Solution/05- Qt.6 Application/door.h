...
public:
    ...
    Q_INVOKABLE bool start( const QString& program, const QVariantList& arguments )
    {
        QStringList args;
        ...
        QProcess::waitForFinished(); // Wait for the process to finish

        return QProcess::exitCode() == 0;
    }

    // If wan to start without arguments for Qt > 5.14
    Q_INVOKABLE bool start( const QString& program )
    {
        QProcess::setProcessChannelMode(QProcess::MergedChannels);
